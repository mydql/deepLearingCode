...
class Sample1(keras.Model):
    def __init__(self):
        ...
        self.conv = layers.Conv2D(25,32,32,padding='valid')

    def call(self, inputs):
        #print(inputs.shape)
        out = self.conv(inputs)
        out = shift_ps(out,5,5)
        return out

class Sample2(keras.Model):
    def __init__(self):
        super(Sample2, self).__init__()
        self.conv = layers.Conv2D(1024,5,5,padding='valid')
...
class Discriminator_w(keras.Model):
    def __init__(self):
        ...
        self.conv21 = layers.Conv2D(32,6,6,padding='same')
        self.conv22 = layers.Conv2D(1,2,1,padding='valid')
    ...
    def call(self, inputs):
        ...
        out1 = self.conv3(out1)#[B,2,2,128]
        out1 = self.flatten1(out1)   #512

        out1 = tf.matmul(out1, self.kernel1)

        out2 = self.conv21(inputs[1])

        out2 = self.conv22(out2)#[B,10,10,1]
...
if __name__ == '__main__':
    x1 = tf.random.truncated_normal([4,10,10,1],dtype=tf.float32)
