...
#     tf.config.experimental.set_virtual_device_configuration(gpu, [tf.config.experimental.VirtualDeviceConfiguration(memory_limit=1024*6)])
...
validation_db = validation_db.batch(32)
...
def main(train = 0):
    ...
    if train == 1:
        ...
        cv2.imwrite('test_recon/x_baby.bmp', tf.squeeze(x).numpy())
