...
class Sample1(keras.Model):
    def __init__(self):
        ...
        self.conv = layers.Conv2D(36,32,32,padding='valid')

    def call(self, inputs):
        #print(inputs.shape)
        out = self.conv(inputs)
        out = shift_ps(out,6,6)
        return out

class Sample2(keras.Model):
    def __init__(self):
        super(Sample2, self).__init__()
        self.conv = layers.Conv2D(1024,6,6,padding='valid')
...
class Discriminator_w(keras.Model):
    ...
    def call(self, inputs):
        ...
        out2 = self.conv22(out2)#[B,12,12,1]
...
if __name__ == '__main__':
    x1 = tf.random.truncated_normal([4,12,12,1],dtype=tf.float32)
