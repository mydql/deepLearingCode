...
class Sample1(keras.Model):
    def __init__(self):
        ...
        self.conv = layers.Conv2D(100,32,32,padding='valid')

    def call(self, inputs):
        #print(inputs.shape)
        out = self.conv(inputs)
        out = shift_ps(out,10,10)
        return out

class Sample2(keras.Model):
    def __init__(self):
        super(Sample2, self).__init__()
        self.conv = layers.Conv2D(1024,10,10,padding='valid')
...
class ResDenseBlock_5C(keras.Model):
    """Residual Dense Block"""
    def __init__(self, nf=32, gc=16, res_beta=0.2,  name='RDB5C',**kwargs):
        super(ResDenseBlock_5C, self).__init__(name=name, **kwargs)
        self.res_beta = res_beta
        self.conv1 = layers.Conv2D(filters=gc,kernel_size=3,padding='SAME', activation=tf.nn.leaky_relu)
        self.conv2 = layers.Conv2D(filters=gc,kernel_size=3,padding='SAME', activation=tf.nn.leaky_relu)
        self.conv3 = layers.Conv2D(filters=gc,kernel_size=3,padding='SAME', activation=tf.nn.leaky_relu)
        self.conv4 = layers.Conv2D(filters=gc,kernel_size=3,padding='SAME', activation=tf.nn.leaky_relu)
        self.conv5 = layers.Conv2D(filters=nf,kernel_size=3,padding='SAME', activation=tf.nn.leaky_relu)
...
class ResInResDenseBlock(keras.Model):
    """Residual in Residual Dense Block"""
    def __init__(self, nf=32, gc=16, res_beta=0.2, name='RRDB',
                 **kwargs):
        super(ResInResDenseBlock, self).__init__(name=name, **kwargs)
        self.res_beta = res_beta
        self.rdb_1 = ResDenseBlock_5C(nf, gc, res_beta=res_beta)
        self.rdb_2 = ResDenseBlock_5C(nf, gc, res_beta=res_beta)
        self.rdb_3 = ResDenseBlock_5C(nf, gc, res_beta=res_beta)
...
class Supplement(keras.Model):
    """Residual in Residual Dense Block"""
    def __init__(self):
        super(Supplement, self).__init__()
        self.conv1 = layers.Conv2D(32,3,1,padding='SAME')
        self.RRDB1 = ResInResDenseBlock()
        self.RRDB2 = ResInResDenseBlock()
        self.RRDB3 = ResInResDenseBlock()
        self.RRDB4 = ResInResDenseBlock()
        self.RRDB5 = ResInResDenseBlock()
        self.conv2 = layers.Conv2D(16,3,1,padding='SAME')
        self.conv3 = layers.Conv2D(1,3,1,padding='SAME')
    ...
    def call(self, x):
        ...
        out = self.RRDB4(out)
        out = self.RRDB5(out)
        out = tf.nn.leaky_relu(self.conv2(out+out1))
        out = tf.nn.leaky_relu(self.conv3(out))
        return out
...
class Discriminator_w(keras.Model):
    def __init__(self):
        ...
        self.conv21 = layers.Conv2D(32,3,2,padding='same')
        self.conv22 = layers.Conv2D(1,13,1,padding='valid')
        ...
        self.kernel1 = tf.Variable(tf.random.truncated_normal([1152, 256], mean=0, stddev=0.1))
        self.kernel2 = tf.Variable(tf.random.truncated_normal([1152, 256], mean=0, stddev=0.1))
...
if __name__ == '__main__':
    x1 = tf.random.truncated_normal([4,20,20,1],dtype=tf.float32)
